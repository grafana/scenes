...
import { getCompareSeriesRefId, getOrigSeriesRefId } from './timeCompare';
...
export const timeShiftAlignmentProcessor: ExtraQueryDataProcessor = (primary, secondary) => {
  ...
  const series = secondary.series.map((frame) => {
    const compareRefId = getCompareSeriesRefId(frame.refId || '');
    const origRefId = getOrigSeriesRefId(compareRefId);
    return {
      ...frame,
      refId: compareRefId,
      meta: {
        ...
        timeCompare: {
          ...
          originalRefId: origRefId, // frame.refId might have been suffixed entering this function, remove it no matter what
        },
      },
    };
  });
