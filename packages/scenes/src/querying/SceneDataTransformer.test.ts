...
describe('SceneDataTransformer', () => {
  ...
  describe('runtime transformations', () => {
    const multiply =
    ...
    const track = (name: string, order: string[]): CustomTransformOperator => {
      return () => (source) => {
        order.push(name);
        return source;
      };
    ...
    it('keeps independent tagged groups when one group is updated or removed', () => {
      const transformationNode = new SceneDataTransformer({ $data: sourceDataNode, transformations: [] });

      transformationNode.upsertRuntimeTransformations({ tag: 'feature:first', transformations: [multiply(2)] });
      transformationNode.upsertRuntimeTransformations({ tag: 'feature:second', transformations: [multiply(3)] });
      ...
      transformationNode.upsertRuntimeTransformations({ tag: 'feature:first', transformations: [multiply(4)] });

      expect(transformationNode.state.data?.series[0].fields[1].values).toEqual([12, 24, 36]);
      expect(transformationNode.getResolvedSystemTransformations().append.map((item) => item.tag)).toEqual([
        'feature:first',
        'feature:second',
      ]);

      const updates = subscribeToStateUpdates(transformationNode);
      transformationNode.removeRuntimeTransformations('feature:missing');

      expect(updates).toHaveLength(0);

      transformationNode.removeRuntimeTransformations('feature:first');
    ...
    it('preserves registration order on update and moves a re-added tag to the end', () => {
      const order: string[] = [];
      const provider = new TestProvider({
        resolve: () => ({
          prepend: [track('provider prepend', order)],
          append: [track('provider append', order)],
        }),
      });
      const transformationNode = new SceneDataTransformer({
        $data: sourceDataNode,
        transformations: [track('saved', order)],
      });

      provider.setState({ child: transformationNode });
      transformationNode.upsertRuntimeTransformations({
        tag: 'feature:first',
        transformations: [track('runtime first', order)],
      });
      transformationNode.upsertRuntimeTransformations({
        tag: 'feature:second',
        transformations: [track('runtime second', order)],
      });

      sourceDataNode.activate();
      transformationNode.activate();

      expect(order).toEqual(['provider prepend', 'saved', 'provider append', 'runtime first', 'runtime second']);

      order.length = 0;
      transformationNode.upsertRuntimeTransformations({
        tag: 'feature:first',
        transformations: [track('runtime first updated', order)],
      });
      ...
      transformationNode.upsertRuntimeTransformations({
        tag: 'feature:first',
        transformations: [track('runtime first re-added', order)],
      });
      ...
      expect(order).toEqual([
        ...
        'provider append',
        'runtime second',
        'runtime first re-added',
      ]);
    });

    it('reports runtime transformations through the system transformation inspection API', () => {
      const provider = new TestProvider({ resolve: () => ({ append: [transformer1config] }) });
      const transformationNode = new SceneDataTransformer({ $data: sourceDataNode, transformations: [] });
      ...
      expect(transformationNode.getResolvedSystemTransformations().append).toEqual([
        { ...transformer1config, origin: 'plugin', position: 'append' },
        {
          ...transformer2config,
          origin: 'table:column-filter',
          position: 'append',
          tag: 'table:column-filter',
        },
      ]);
    ...
    it('applies changes made while inactive on the next activation', () => {
      const transformationNode = new SceneDataTransformer({ $data: sourceDataNode, transformations: [] });
      ...
      transformationNode.upsertRuntimeTransformations({ tag: 'feature:owner', transformations: [multiply(2)] });
      const deactivateWithRuntime = transformationNode.activate();

      expect(transformationNode.state.data?.series[0].fields[1].values).toEqual([2, 4, 6]);

      deactivateWithRuntime();
      transformationNode.removeRuntimeTransformations('feature:owner');
      transformationNode.activate();

      expect(transformationNode.state.data?.series[0].fields[1].values).toEqual([1, 2, 3]);
    });

    it('keeps runtime contributions out of state, serialization, and clones', () => {
      const transformationNode = new SceneDataTransformer({
        $data: sourceDataNode,
        transformations: [transformer1config],
      });
      ...
      transformationNode.upsertRuntimeTransformations({
        tag: 'table:column-filter',
        transformations: [multiply(3)],
      });
      sourceDataNode.activate();
      transformationNode.activate();

      expect(transformationNode.state.transformations).toEqual([transformer1config]);
      expect(JSON.stringify(transformationNode.toJSON())).not.toContain('table:column-filter');
      expect(transformationNode.state.data?.series[0].fields[1].values).toEqual([6, 12, 18]);
    ...
    it('reprocesses current query results without running the query again', async () => {
      ...
      transformationNode.upsertRuntimeTransformations({ tag: 'feature:owner', transformations: [multiply(3)] });
      await new Promise((resolve) => setTimeout(resolve, 1));
