...
- Non-obvious version requirement: the committed default `GRAFANA_VERSION` in `docker-compose.yaml` (`11.1.0-181853`) is too old — the app's build externalizes `react/jsx-runtime`, which Grafana only exposes as a shared plugin dependency starting in Grafana 13. On older versions the app shows "App not found" with a `react/jsx-runtime` 404. Run against a Grafana 13 (close to `main`) dev image, e.g.:
  `cd packages/scenes-app && sudo GRAFANA_VERSION=13.1.0-25957875840 docker compose up -d --build`
  (find newer `grafana/grafana-dev` tags on Docker Hub if needed).
